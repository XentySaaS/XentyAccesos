...
class RegistroAcceso(models.Model):  # access_logs
    ...
    asistente = models.ForeignKey(
        "citas.AsistenteCita", on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    cajon = models.ForeignKey(
        "eventos.CajonParking", on_delete=models.SET_NULL, null=True, blank=True
    )
    tipo_acceso = models.CharField(
        max_length=10, choices=TipoAcceso.choices, default=TipoAcceso.ENTRADA
    )
...
class RegistroAccesoParking(models.Model):  # access_log_parkings
    ...
    cajon = models.ForeignKey(
        "eventos.CajonParking", on_delete=models.CASCADE, related_name="registros"
    )
    tipo_acceso = models.CharField(
        max_length=10, choices=TipoAcceso.choices, default=TipoAcceso.ENTRADA
    )
