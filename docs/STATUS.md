...
| Hub de proveedores (anti-enumeración) | ✔ La membresía email→tenants **no se revela sin probar propiedad del correo**: código de 6 dígitos (cache Redis, solo hash SHA-256, TTL 10 min, máx 5 intentos con `compare_digest`, cooldown 3 códigos/h/correo) + cookie de dispositivo firmada (90 d, HttpOnly, SameSite=Lax); rate limit 10/m/IP; tenants suspendidos/cancelados y cuentas con baja lógica excluidos. **Decisión de producto (2026-07-21):** el paso 1 sí revela la EXISTENCIA de cuenta (`registrado` booleano, para no mandar a un callejón sin salida a quien no tiene cuenta); lo sensible (en QUÉ recintos) sigue detrás del código. El hub NUNCA maneja contraseñas ni tokens: el login ocurre en el panel del tenant (JWT/aislamiento intactos) |
